...
class UserContextDB {
  ...
  createNewUserContext(userId) {
    return {
      ...
      relacionamento_nocturnus: {
        nivel_intimidade: 1,
        apelido_nocturnus: null,
        memorias_especiais: [],
        conversas_marcantes: [],
        sentimento: 'neutro'
      },
  ...
  /**
   * Atualiza o relacionamento com NOCTURNUS
   */
  updateRelationship(userId, campo, valor) {
    const context = this.getUserContext(userId);
    
    if (context.relacionamento_nocturnus.hasOwnProperty(campo)) {
      context.relacionamento_nocturnus[campo] = valor;
      context.ultima_atualizacao = getBrazilDateTime();
      this.saveDatabase();
    }
  }
  ...
  addSpecialMemory(userId, memoria) {
    ...
    context.relacionamento_nocturnus.memorias_especiais.push(novaMemoria);
    
    // Manter apenas as 30 memórias mais especiais
    if (context.relacionamento_nocturnus.memorias_especiais.length > 30) {
      context.relacionamento_nocturnus.memorias_especiais = 
        context.relacionamento_nocturnus.memorias_especiais.slice(-30);
    }
  ...
  updateMemory(userId, tipo, valorAntigo, valorNovo) {
    ...
    switch (tipoNormalizado) {
      ...
      case 'memória':
        const indexMemoria = context.relacionamento_nocturnus.memorias_especiais.findIndex(
          m => m.texto === valorAntigo
        );
        if (indexMemoria !== -1) {
          context.relacionamento_nocturnus.memorias_especiais[indexMemoria].texto = valorNovo;
          context.relacionamento_nocturnus.memorias_especiais[indexMemoria].data = getBrazilDateTime();
          atualizado = true;
        }
        break;
  ...
  deleteMemory(userId, tipo, valor) {
    ...
    switch (tipoNormalizado) {
      ...
      case 'memória':
        ...
        if (indexMemoria !== -1) {
          context.relacionamento_nocturnus.memorias_especiais.splice(indexMemoria, 1);
          removido = true;
        }
        break;
  ...
  getUserContextSummary(userId) {
    ...
    const summary = {
      ...
      nivel_intimidade: context.relacionamento_nocturnus.nivel_intimidade,
      ...
      memorias_especiais: context.relacionamento_nocturnus.memorias_especiais.slice(-5).map(m => m.texto).join('\n- ') || 'Nenhuma'
    };
